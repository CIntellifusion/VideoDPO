...
def technical_quality(model, video_list, device, **kwargs):
    ...
    for video_path in tqdm(video_list,position=device,leave=True):
        ...
        images = transform(images, preprocess_mode).to(device)
        acc_score_video = 0.0
        ...
        ### add a test 
        batch_score = float(model(images).sum())
        # print(batch_score,acc_score_video);exit()
        acc_score_video=batch_score # slight different 
