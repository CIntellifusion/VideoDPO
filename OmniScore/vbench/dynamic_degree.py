...
class DynamicDegree:
    ...
    def infer(self, video_path):
        with torch.no_grad():
            ...
            # print(len(frames))
            static_score = np.array(static_score)
            return self.move_rate(static_score)
            # whether_move = self.check_move(static_score)
            # return whether_move
    def move_rate(self, score_list):
        thres = self.params["thres"]
        count_num = self.params["count_num"]
        
        # 计算超过阈值的帧数
        count = np.sum(score_list > thres)
        
        # 计算占总帧数的比例
        # print("in move rate, score_list.size:",score_list.size)
        total_frames = score_list.size
...
def dynamic_degree(dynamic, video_list, device):
    # 0903 modification , add device as position 
    sim = []
    video_results = []
    for video_path in tqdm(video_list, position=device, leave=True):
...
def compute_dynamic_degree(json_dir, device, submodules_list, **kwargs):
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print("warning!! You are calling customed dynamic function!!!!")
    print("instead of return a Bool Value, this function returns a avg move frame rate.")
    print("refer to self.move_score in this script vbench/dynamic_degree.py for details!")
    ...
    all_results, video_results = dynamic_degree(dynamic, video_list, device)
